...
// Must match SeedPage password formula — creates/retrieves Supabase Auth user
// so users.id FK constraint is satisfied.
async function getOrCreateAuthUserId(email) {
  const pw = email.replace('@demo.com', '').replace(/[^a-zA-Z0-9]/g, '') + '_EmbarkGCC2025!';
  const { data: up } = await supabase.auth.signUp({ email, password: pw });
  ...
  throw new Error(`Cannot auth ${email}. Delete this user in Supabase Auth dashboard and retry.`);
}

async function runSeed(log) {
  // ── AUTH USERS (needed for FK constraint) ──────────────────────────────────
  log('Setting up Supabase Auth users…');
  ...
  for (const project of projects) {
    for (let n = 1; n <= 6; n++) {
      ...
      const { data: stage, error: sErr } = await supabase.from('stages').insert({
        project_id: project.id, stage_number: n, stage_name: STAGE_NAMES[n - 1],
        status, completion_percentage: pct, started_at: startedAt, completed_at: completedAt,
      }).select().single();
      if (sErr) { log('⚠️ Stage warn: ' + sErr.message); continue; }
      const gates = (STAGE_GATES[n] || []).map(item => ({
        stage_id: stage.id, gate_item: item, is_required: true,
  ...
  log('✅ 18 stages + 54 gate checklist items created');
  ...
  const msSLA      = new Date(); msSLA.setDate(msSLA.getDate() + 4);
  ...
  const { data: escs, error: eErr } = await supabase.from('escalations').insert([
    ...
    },
    {
      ...
      severity: 'Medium', status: 'Open', assigned_to: PMO_USER_2_ID,
      sla_due_date: msSLA.toISOString(), created_at: daysAgo(1),
    },
  ]).select();
  ...
  // ── ESCALATION COMMENTS ────────────────────────────────────────────────────
  log('Inserting escalation comment threads…');
  ...
  else log('✅ 11 escalation comment threads created');
  ...
  log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  log('Seed complete!');
  log('  PMO   : pmo@demo.com    / pmo     → sees all 3 projects');
  log('  PMO2  : pmo2@demo.com   / pmo2    → sees all 3 projects');
  log('  Client: client@demo.com / client  → JPMorgan India GCC');
  log('  Client: client2@demo.com/ client2 → Goldman Sachs Tech Hub');
  log('  Client: client3@demo.com/ client3 → Microsoft Innovation Centre');
  log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
}
    ...
    <div style={{ minHeight: '100vh', background: '#0f172a', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div style={{ maxWidth: 680, width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          <div style={{ fontSize: '2rem', fontWeight: 800, color: 'white', marginBottom: '0.5rem' }}>Embark — Database Seed</div>
          <p style={{ color: '#94a3b8', fontSize: '0.9375rem' }}>Clears all tables and re-populates the full demo dataset across 3 GCC projects and 5 users.</p>
        </div>

        <div style={{ background: '#1e293b', borderRadius: 16, padding: '2rem', marginBottom: '1.5rem' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
            {[{ label: 'PMO Users', value: '2', sub: 'Sarah · Rahul' },{ label: 'Client Users', value: '3', sub: 'Michael · David · Priya' },{ label: 'GCC Projects', value: '3', sub: 'JPM · GS · MSFT' }].map(c => (
          ...
          <button
            ...
            style={{ width: '100%', padding: '1rem', borderRadius: 12, border: 'none', background: status === 'running' ? '#334155' : status === 'done' ? '#10b981' : '#6366f1', color: 'white', fontSize: '1rem', fontWeight: 700, cursor: status === 'running' ? 'not-allowed' : 'pointer' }}
          >
            {status === 'idle'    ? '▶ Run Seed — Clear & Re-populate All Tables' :
             status === 'running' ? '⏳ Seeding in progress…' :
             status === 'done'    ? '✅ Seed Complete — Go to Login (/)' :
          ...
          {status === 'error' && (
            <div style={{ marginTop: '1rem', background: '#1a0a0a', border: '1px solid #7f1d1d', borderRadius: 8, padding: '0.75rem', fontSize: '0.8125rem', color: '#fca5a5' }}>
              <strong>Re-seed tip:</strong> Go to <strong>Supabase → Authentication → Users</strong>, delete the 5 demo email accounts, then click Run Seed again.
            </div>
          )}
          ...
          {status === 'done' && (
            <div style={{ marginTop: '1rem', display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
              {[['pmo@demo.com','pmo'],['pmo2@demo.com','pmo2'],['client@demo.com','client'],['client2@demo.com','client2'],['client3@demo.com','client3']].map(([e,p]) => (
                <div key={e} style={{ flex: 1, minWidth: 100, background: '#0f172a', borderRadius: 8, padding: '0.5rem', textAlign: 'center' }}>
                  <div style={{ fontSize: '0.625rem', color: '#94a3b8' }}>{e}</div>
        ...
        {logs.length > 0 && (
          <div style={{ background: '#0f172a', borderRadius: 12, padding: '1.25rem', fontFamily: 'monospace', fontSize: '0.8125rem', maxHeight: 400, overflowY: 'auto', border: '1px solid #1e293b' }}>
            {logs.map((l, i) => (
              <div key={i} style={{ marginBottom: 2, color: l.startsWith('✅') ? '#10b981' : l.startsWith('⚠️') ? '#f59e0b' : l.startsWith('❌') ? '#ef4444' : l.startsWith('━') ? '#475569' : '#94a3b8' }}>
                {l}
              </div>
            ))}
          </div>
